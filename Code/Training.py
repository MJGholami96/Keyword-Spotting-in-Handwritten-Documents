...
class DetectionConfig(Config):
    ...
    NAME = "history_250_epoch 15_200steps_Edited_cfg"
    # number of classes (background + history)
    NUM_CLASSES = 1 + 1
...
config.display()
# define the model
model = MaskRCNN(mode='training', model_dir=r'F:\project\code\Mask R-CNN\Learning Weights', config=config)
# load weights (mscoco) and exclude the output layers
model.load_weights(r'F:\Mask R-CNN\mask_rcnn_coco.h5', by_name=True, exclude=["mrcnn_class_logits", "mrcnn_bbox_fc",  "mrcnn_bbox", "mrcnn_mask"])
